...
//        sb.append(String.format("len: %02d", len));
        sb.append(String.format(Locale.ENGLISH,"@Len = %02X, @Type = 0x%02X", len, type));
        ...
        try {
            for (byte b : bytes) {
                sbSub.append(String.format(Locale.ENGLISH,format, b & 0xff));
            }
            sb.append(sbSub.toString());
        } catch (Exception e) {
            sb.append(ByteUtil.bytesToHex(bytes));
        }
